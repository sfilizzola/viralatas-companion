...
**Realtime enabled on:** `user_picks`, `announcements`, `user_presence`, `metal_place_config`, `metal_place_windows`, `live_band_test_config`, `festival_memberships`
...
**Current phase: 49** — Announcement Lineup (`running_order` flag). See `PHASES.md`. Upcoming: 50 `/now` planning mode · 51 My Picks planning mode.
...
4. The prefix `v1.5` is hardcoded and never changes; only the patch number increments
...
- `PHASES.md` — Current phase **49** (Announcement Lineup): acceptance criteria, deliverables
