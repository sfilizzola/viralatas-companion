...
import styles from './QuackStrip.module.css';

type QuackStripProps = {
  onDuck: () => void;
  /** Expiry timestamp (ms since epoch) of the current cooldown, or null when idle. */
  cooldownUntil: number | null;
};

export default function QuackStrip({ onDuck, cooldownUntil }: QuackStripProps) {
  const { t } = useI18n('QuackStrip');
