...
describe('bandDay', () => {
  it('returns the CEST calendar date for a midday band (14:00 CEST)', () => {
    ...
    expect(bandDay('2026-07-29T12:00:00Z')).toBe('2026-07-29');
  });
  ...
  it('rolls back to previous day when CEST hour is 2 (00:00 UTC → 02:00 CEST)', () => {
    ...
    expect(bandDay('2026-07-30T00:00:00Z')).toBe('2026-07-29');
  });
  ...
  it('rolls back at CEST midnight (22:00 UTC previous day = 00:00 CEST)', () => {
    ...
    expect(bandDay('2026-07-29T22:00:00Z')).toBe('2026-07-29');
  });
  ...
  it('rolls back at 03:59 CEST but not at 04:00 CEST (the boundary)', () => {
    ...
    expect(bandDay('2026-07-30T01:59:00Z')).toBe('2026-07-29');

    // 02:00 UTC on July 30 = 04:00 CEST → hour 4, NOT < 4 → stays July 30
    expect(bandDay('2026-07-30T02:00:00Z')).toBe('2026-07-30');
  });
  ...
  it('maps to Day 2 (2026-07-30) for bands at 14:00 CEST on July 30', () => {
    expect(bandDay('2026-07-30T12:00:00Z')).toBe('2026-07-30');
  });

  it('maps to Day 3 (2026-07-31) for bands at 14:00 CEST on July 31', () => {
    expect(bandDay('2026-07-31T12:00:00Z')).toBe('2026-07-31');
  });

  it('maps to Day 4 (2026-08-01) for bands at 14:00 CEST on August 1', () => {
    expect(bandDay('2026-08-01T12:00:00Z')).toBe('2026-08-01');
  });
  ...
  it('handles a time outside the festival range (still returns a valid CEST date)', () => {
    ...
    expect(bandDay('2026-06-01T10:00:00Z')).toBe('2026-06-01');
  });
});

describe('bandWeekdayKey', () => {
  it('returns wednesday for Day 1 (2026-07-29)', () => {
    expect(bandWeekdayKey('2026-07-29T12:00:00Z')).toBe('wednesday');
  });

  it('follows bandDay rollover for after-midnight slots', () => {
    // 00:00 UTC July 30 → CEST rolls to July 29 → wednesday
    expect(bandWeekdayKey('2026-07-30T00:00:00Z')).toBe('wednesday');
  });
});
