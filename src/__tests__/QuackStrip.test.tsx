...
import QuackStrip from '../components/QuackStrip';
...
describe('QuackStrip', () => {
  ...
  it('calls onDuck when clicked and not on cooldown', () => {
    ...
    const onDuck = vi.fn();
    render(<QuackStrip onDuck={onDuck} cooldownUntil={null} />);
  ...
  it('is disabled and does not call onDuck when on cooldown', () => {
    ...
    const onDuck = vi.fn();
    render(<QuackStrip onDuck={onDuck} cooldownUntil={Date.now() + 90_000} />);
  ...
  it('shows "quack" label when idle', () => {
    vi.mocked(useCooldown).mockReturnValue(false);
    render(<QuackStrip onDuck={() => {}} cooldownUntil={null} />);
    expect(screen.getByText('quack')).toBeInTheDocument();
  });

  it('shows "cooldown" label when on cooldown', () => {
    vi.mocked(useCooldown).mockReturnValue(true);
    render(<QuackStrip onDuck={() => {}} cooldownUntil={Date.now() + 90_000} />);
  ...
  it('renders the duck image', () => {
    vi.mocked(useCooldown).mockReturnValue(false);
    render(<QuackStrip onDuck={() => {}} cooldownUntil={null} />);
    // The duck is decorative (alt="") so it is absent from the a11y tree by
    // role — query the element directly to assert the asset path.
